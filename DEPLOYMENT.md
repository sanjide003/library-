...
This app is currently a static Firebase-powered frontend. The deployable website root is the `public/` directory.
...
Firebase web config in `public/js/firebase-config.js` is not a server secret, but production must still use:
...
6. Ensure Output Directory is `public`. The included `vercel.json` already sets this.
7. Deploy.
...
The repository includes `.github/workflows/deploy-github-pages.yml` to deploy the `public/` directory.
...
5. GitHub Actions uploads the `public/` directory and publishes it to Pages.
6. Add the GitHub Pages domain in Firebase Authentication authorized domains.

### GitHub Pages Result

The action deploys only the `public/` directory, so URLs should be:
...
The repository includes `firebase.json` with `public` as the hosting root.
...
Firebase Hosting serves the files from `public/`, and Firestore rules can be deployed from the same repository.
...
| --- | --- | --- |
| Vercel | Fastest deploy with your existing workflow | Ready: `vercel.json` added |
| GitHub Pages | Free GitHub-based static publishing | Ready: GitHub Actions workflow added |
