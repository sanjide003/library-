...
- [x] Restored root-level static HTML publishing.
  - Purpose: Make GitHub Pages and simple static hosts automatically serve `index.html` from the repository root.
  - Verification: `index.html`, `admin.html`, and `reader.html` are root-level files and still import `./js/config.js`.
  - Remaining risk: Manual browser smoke testing is still needed with a configured Firebase project.
- [x] Split Firebase web configuration into `js/firebase-config.js`.
